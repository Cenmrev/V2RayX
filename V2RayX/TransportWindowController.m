...
- (BOOL)checkInputs {
    ...
    if (httpHeaderParseError) {
        ...
        [parseAlert setMessageText:NSLocalizedString(@"Error in parsing customized tcp http header!", @"tcp http 配置出错")];
    ...
    if(wsHeaderParseError) {
        NSAlert* parseAlert = [[NSAlert alloc] init];
        [parseAlert setMessageText:NSLocalizedString(@"Error in parsing customized WebSocket headers!", @"WebSocket配置出错")];
        [parseAlert beginSheetModalForWindow:[self window] completionHandler:^(NSModalResponse returnCode) { }];
        return NO;
    }
    return YES;
